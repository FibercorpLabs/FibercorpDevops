...
def get_args():
    ...
    parser.add_argument(
        '-s', '--host', required=False, action='store',
        help='Remote host to connect to')
    parser.add_argument(
        '-o', '--port', type=int, default=443, action='store',
        help='Port to connect on')
    parser.add_argument(
        '-u', '--user', required=False, action='store',
        help='User name to use when connecting to host')
    parser.add_argument(
        '-p', '--password', required=False, action='store',
        ...
        for f in rs.file:
            try:
                ...
                vmxurl = "https://%s/folder/%s%s?dcPath=%s&dsName=%s" % \
                         ("10.120.4.100", vmfold, dsfile, datacenter, fulldsname)
...
def examine_vmx(dsname):
    ...
    try:
        ...
        for file_vmx in VMX_PATH:
            ...
            username = "aliguori@int.fibercorp.com.ar"
            password = "Desarma21!"
...
def main():
    ...
    try:
        si = None
        try:
            si = connect.SmartConnect(host="10.120.4.100",
                              user="aliguori@int.fibercorp.com.ar",
                              pwd="Desarma21!",
                              port=443,
                              sslContext=context)
        except IOError, e:
